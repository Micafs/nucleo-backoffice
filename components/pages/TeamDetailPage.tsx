...
import { Avatar, RolePill, StatusDot } from '@/components/ui';
import { TEAMS, PERMISSION_GROUPS, ROLE_DEFAULTS, getTeam, User } from '@/lib/data';
...
export default function TeamDetailPage({
  teamId, users, onNavigate, onInvite,
  permView, setPermView, permissions, setPermissions,
}: TeamDetailPageProps) {
  const team = getTeam(teamId);
  const [tab, setTab] = useState<'members' | 'permissions' | 'settings'>('members');
            ...
            {members.map((u) => {
              ...
              return (
                <div key={u.id} className="member-row">
                  ...
                  <RolePill role={membership?.role || 'member'} />
...
interface PermissionsEditorProps {
  ...
  variant: PermView;
  onChangeVariant: (v: PermView) => void;
  permissions: PermissionsState;
  setPermissions: (p: PermissionsState) => void;
}

export function PermissionsEditor({
  team, selectedRole, setSelectedRole, variant, onChangeVariant, permissions, setPermissions,
  ...
  const toggle = (permId: string) => {
    ...
    setPermissions({ ...permissions, [teamKey]: { ...(permissions[teamKey] || {}), [selectedRole]: next } });
  };
  const has = (permId: string) => rolePerms.includes(permId);
        ...
        {variant !== 'matrix' && (
          <div className="row" style={{ marginBottom: 16, flexWrap: 'wrap' }}>
            ...
            {['admin', 'member', 'viewer'].map((r) => (
              <button key={r} className={'btn btn--sm' + (selectedRole === r ? ' btn--primary' : '')} onClick={() => setSelectedRole(r)}>
...
function PermissionsMatrix({ team, permissions, setPermissions }: {
  team: ReturnType<typeof getTeam>;
  permissions: PermissionsState;
  setPermissions: (p: PermissionsState) => void;
}) {
  const teamKey = team.id;
  const getPerms = (role: string) => permissions[teamKey]?.[role] || ROLE_DEFAULTS[role] || [];
  const toggle = (role: string, permId: string) => {
    ...
    setPermissions({ ...permissions, [teamKey]: { ...(permissions[teamKey] || {}), [role]: next } });
  };
  return (
...
function PermissionsGroups({ has, toggle }: { has: (id: string) => boolean; toggle: (id: string) => void }) {
  return (
    <>
      {PERMISSION_GROUPS.map((group) => {
...
function IndeterminateCheckbox({ checked, indeterminate, onChange }: {
  ...
  return <input ref={ref} type="checkbox" className="checkbox" checked={checked} onChange={onChange} />;
}

function PermissionsCards({ has, toggle }: { has: (id: string) => boolean; toggle: (id: string) => void }) {
  const allPerms = PERMISSION_GROUPS.flatMap((g) => g.permissions.map((p) => ({ ...p, groupIcon: g.icon })));
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 12 }}>
      {allPerms.map((p) => (
        <div key={p.id} className="perm-card">
          ...
          <button className="switch" data-on={String(has(p.id))} onClick={() => toggle(p.id)}><i /></button>
        </div>
      ))}
    </div>
  );
